-- Reconciliação de schema: objetos que existiam no banco mas não nas migrations.
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS galoes_troca_refil integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.orders.galoes_troca_refil IS 'Galões enviados para troca/refil neste pedido.';

CREATE OR REPLACE FUNCTION public.create_order_with_items(_order jsonb, _items jsonb, _payments jsonb DEFAULT '[]'::jsonb)
...
REVOKE ALL ON FUNCTION public.create_order_with_items(jsonb, jsonb, jsonb) FROM anon, public;
